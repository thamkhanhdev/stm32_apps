...
AVISTATUS __AVI_Init( AVI_CONTEXT * pavi, uint8_t *buf, uint32_t size)
{
  ...
  if(aviheader->ChunkID != AVI_RIFF_ID)
  {
    return AVI_RIFF_ERR;
  }

  if(aviheader->ChunkListID != AVI_AVI_ID)
  {
    return AVI_AVI_ERR;
  }
  ...
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U,  7U,  0xFFF, "1. %u\r\n", pavi->aviInfo.SecPerFrame);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 14U,  0xFFF, "2. %u\r\n", pavi->aviInfo.TotalFrame);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 21U,  0xFFF, "3. %.4s\r\n", pavi->aviInfo.VideoFLAG);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 28U,  0xFFF, "4. %.4s\r\n", pavi->aviInfo.AudioFLAG);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 35U,  0xFFF, "5. %u\r\n", pavi->aviInfo.Width);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 42U,  0xFFF, "6. %u\r\n", pavi->aviInfo.Height);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 49U,  0xFFF, "7. %u\r\n", pavi->aviInfo.SampleRate);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 56U,  0xFFF, "8. %u\r\n", pavi->aviInfo.Channels);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U,  128U, 63U,  0xFFF, "9. %u\r\n", pavi->aviInfo.AudioType);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U, 172U,  7U,  0xFFF, "1. %u\r\n", pavi->aviInfo.AudioBufSize);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U, 42U, 14U,  0xFFF, "2. %u\r\n", pavi->dwStart);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U, 42U, 21U,  0xFFF, "3. %u\r\n", pavi->dwLength);
  // MATRIX_Printf( FONT_TOMTHUMB, 1U, 42U, 28U,  0xFFF, "4. %u\r\n", pavi->dwSuggestedBufferSize);
...
uint32_t AVI_GetFrame(AVI_CONTEXT * pavi, FIL *file)
{
  ...
  if(pavi->CurrentImage== 0 )
  {
    ...
    // MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "1.");
    // HAL_Delay(500);
    f_lseek(file, 0 ); /* Go to the file start */

    // MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "2.");
    // HAL_Delay(500);
    /* Read data*/
    f_read(file, pavi->pVideoBuffer, 10000, (UINT*)&readSize );

    // MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "3:%u-%u\r\n", readSize, 10000);
    // HAL_Delay(500);
    /* Check for "movi" tag */
    offset = __AVI_SearchID(pavi->pVideoBuffer,10000,(uint8_t*)"movi");
    // MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "4.%u", offset);
    // HAL_Delay(1000);
    /* Read first Frame info*/
    result = __AVI_GetStreamInfo(pavi, pavi->pVideoBuffer + offset +4);
    // MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "5.%u", result);
    // HAL_Delay(500);
  ...
  if(pavi->aviInfo.StreamID  ==  AVI_VIDS_FLAG)
  {
    ...
    offset += 8;
    f_lseek(file, offset );
    MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "6.%d ", offset);
    HAL_Delay(1000);

    f_read(file, Test_AudioBuffer, 8 , (UINT*)&readSize );
    offset += 8;
    f_lseek(file, offset );
    MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "7.%d ", offset);
    HAL_Delay(1000);
    f_read(file, Test_AudioBuffer, 8 , (UINT*)&readSize );

    MATRIX_Printf( FONT_DEFAULT, 1U, 0xFFFF, 0xFFFF, 0xFFFF, "8. ");
    HAL_Delay(500);
